...
# ATOM Matrix
pio run -e atom-matrix-calibrate -t upload

# Waveshare S3
pio run -e waveshare-s3-calibrate -t upload

# M5StickC Plus
pio run -e m5stickc-plus-calibrate -t upload

# LilyGO T-Display
pio run -e lilygo-t-display-calibrate -t upload

# AIPI-Lite
pio run -e aipi-lite-calibrate -t upload
```
...
### Data Collection
The tool reads raw accelerometer values at each rotation and correlates them with user-reported marker positions. The calculation algorithm (TODO) will:
...
- [ ] Implement automatic calculation algorithm
- [ ] Add validation step (test all 4 orientations before saving)
- [ ] Optional: Save to NVS for runtime override
- [ ] Support for accelerometer-only boards (no gyro)

## References

- See `IMU_Analysis.md` for detailed problem analysis
- Board configs: `include/BoardConfigs/`
- IMU drivers: `src/Hardware/Sensors/`

---

**Note**: This is a one-time calibration for hardware bring-up. Once values are in the board config, they're committed to git and survive flash erases.
