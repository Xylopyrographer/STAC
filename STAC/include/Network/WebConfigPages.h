...
<body>
  <div class="container">
    ...
    <div id="portal-notice" class="info-text" style="display:none; margin: 15px 20px; background: #fff3cd; border: 1px solid #ffc107;">
      ...
      <form id="form-v80hd" method="post" action="/config" style="display:none;">
        ...
        <div class="section">
          ...
          <input type="number" id="stChanHDMI3" name="stChanHDMI" value="4" min="1" max="4" inputmode="numeric" pattern="[0-9]*" required>
          
          <label for="stChanSDI3">Max SDI Channel (1-4):</label>
          <input type="number" id="stChanSDI3" name="stChanSDI" value="4" min="1" max="4" inputmode="numeric" pattern="[0-9]*" required>
    ...
    function updateExportJSON() {
      ...
      const config = {
        ...
        switch: {
          ...
          pollInterval: parseInt(document.getElementById(pollField).value)
        }
      };
    ...
    // Load from clipboard with validation (modern API with fallback)
    function loadFromClipboard() {
      // Try modern Clipboard API first (works on desktop HTTPS/localhost)
      if (navigator.clipboard && navigator.clipboard.readText) {
        navigator.clipboard.readText()
          ...
          .catch(function(error) {
            // If modern API fails, fall back to execCommand approach
            console.log('Clipboard API failed, using fallback:', error.message);
            useExecCommandFallback();
          });
      } else {
        // Fallback for browsers without Clipboard API (mobile)
        useExecCommandFallback();
      }
    }
    
    // Fallback method using paste event listener (works on all browsers)
    ...
    function processClipboardData(jsonStr) {
    ...
    function applyConfig(config) {
      ...
      if (config.model === 'V-60HD') {
        ...
        document.getElementById('stnetUser').value = config.switch.lanUsername || 'user';
        document.getElementById('stnetPW').value = config.switch.lanPassword || '0000';
    ...
    function copyGeekInfo() {
      ...
      try {
        document.execCommand('copy');
        document.body.removeChild(textarea);
        alert('Information copied to the clipboard.');
      } catch (error) {
        document.body.removeChild(textarea);
        alert('Copy failed: ' + error.message);
      }
    ...
    <div id="geek-modal" class="modal">
      <div class="modal-content">
        <h3 style="margin-top: 0; color: #607d8b;">STAC Info</h3>
...
<body>
  <div class="container">
    ...
    <p><strong>Remove power or</strong></p>
    <p><strong>Press the reset button to setup the device.</strong></p>
  </div>
</body>
</html>
)=====";
