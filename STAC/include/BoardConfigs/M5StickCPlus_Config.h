...
// Display dimensions (physical visible area)
#define DISPLAY_WIDTH 135
#define DISPLAY_HEIGHT 240

// ST7789 controller internal framebuffer dimensions
// The controller has 240×320 memory, physical display is a viewport into this
#define TFT_MEMORY_WIDTH 240
#define TFT_MEMORY_HEIGHT 320

// Panel offsets (for ST7789 135x240 centering in 240x320 memory)
// NOTE: ST7789 has 240x320 internal memory but M5StickC Plus is 135x240
// Offsets change per rotation to map the visible window correctly
...
// Offsets calculated as: (framebuffer_dimension - physical_dimension) / 2
...
// TFT rotation offset - compensates for physical LCD mounting orientation
// LovyanGFX rotation: 0=0°, 1=90° CW, 2=180°, 3=270° CW
// This offset is added to the IMU-detected orientation to get correct display rotation
#define TFT_ROTATION_OFFSET 0  // Adjust if display appears rotated at startup
...
/**
 * Backlight polarity - controlled via AXP192, not direct GPIO
 * Not applicable for PMU-controlled backlights
 */
// #define TFT_BACKLIGHT_ON HIGH  // N/A - PMU controlled
...
// IMU axis remapping (calibrated 2026-01-15)
// These define how to map raw IMU axes to board coordinate system
#define IMU_AXIS_REMAP_X    (acc.x)
#define IMU_AXIS_REMAP_Y    (acc.y)
#define IMU_AXIS_REMAP_Z    (acc.z)

// Orientation mappings for M5StickCPlus (calibrated 2026-01-15)
// Home pattern #1, corner offset 0 (baseline LUT #0)
// These map device orientation enums to appropriate rotation LUTs
...
// Reverse mapping for debug logging: enum → physical angle (calibrated 2026-01-15)
...
// GPIO pins drive the GROVE connector with 2-bit tally state
...
#define HAS_PERIPHERAL_MODE_CAPABILITY true
    ...
    #define PIN_TALLY_STATUS_0 32   // GROVE pin 0 (LSB of tally state)
    #define PIN_TALLY_STATUS_1 33   // GROVE pin 1 (MSB of tally state)
