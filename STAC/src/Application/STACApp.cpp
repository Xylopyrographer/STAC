...
#ifdef GLYPH_SIZE_5X5
        const uint8_t *earlyPowerGlyph = Display::Glyphs5x5::BASE_GLYPHS[ Display::Glyphs5x5::GlyphIndex::GLF_PO ];
#else
        const uint8_t *earlyPowerGlyph = Display::Glyphs8x8::BASE_GLYPHS[ Display::Glyphs8x8::GlyphIndex::GLF_MID ];
#endif
    ...
    void STACApp::handleNormalMode() {
        ...
        if ( !startupConfigDone ) {
            ...
            if ( ops.autoStartEnabled ) {
                ...
                using namespace Glyphs8x8::GlyphIndex;
#endif
                const uint8_t *cornersGlyph = glyphManager->getGlyph( GLF_CORNERS );
                ...
                while ( millis() < autostartTimeout ) {
                    ...
                    if ( millis() >= nextFlash ) {
                        ...
                        // Pulse corners (on or off based on state)
