...
namespace Storage {
    ...
    String ConfigManager::obfuscatePassword( const String &password ) {
        // Handle empty password
        if ( password.isEmpty() ) {
            return "";
        }

        // Get device MAC address to use as XOR key
        uint8_t mac[ 6 ];
        esp_efuse_mac_get_default( mac );

        // XOR each character with cycling MAC bytes, then hex-encode
        String obfuscated;
        obfuscated.reserve( password.length() * 2 );  // Each byte becomes 2 hex chars

        for ( size_t i = 0; i < password.length(); i++ ) {
            uint8_t obfuscatedByte = password[ i ] ^ mac[ i % 6 ]; // Cycle through MAC bytes
            // Convert to 2-digit hex
            if ( obfuscatedByte < 0x10 ) {
                obfuscated += '0';
            }
            obfuscated += String( obfuscatedByte, HEX );
        }

        return obfuscated;
    }

    String ConfigManager::deobfuscatePassword( const String &obfuscated ) {
        // Handle empty obfuscated string
        if ( obfuscated.isEmpty() ) {
            return "";
        }

        // Get device MAC address (same key used for obfuscation)
        uint8_t mac[ 6 ];
        esp_efuse_mac_get_default( mac );

        // Decode hex pairs back to bytes, then XOR with cycling MAC
        String password;
        password.reserve( obfuscated.length() / 2 );

        for ( size_t i = 0; i < obfuscated.length(); i += 2 ) {
            // Parse hex byte
            String hexByte = obfuscated.substring( i, i + 2 );
            uint8_t obfuscatedByte = ( uint8_t )strtol( hexByte.c_str(), nullptr, 16 );

            // XOR with MAC to recover original character
            char originalChar = obfuscatedByte ^ mac[ ( i / 2 ) % 6 ];
