 ...
 *
 *      For the V160:
 *          stClient.print("GET /tally/" +  "bank" + BANK_NUM + "/status\r\n\r\n");   // Uses the short form of the tally status request
 *
...
*/
    ...
    // but need to first set the channel query to the right format for the V160
    if ( tally.tChannel < 9 ) {
        bank = "hdmi_";
        bankChan = tally.tChannel;
    }
    else {
        bank = "sdi_";
        bankChan = tally.tChannel - 8;
    }
    stClient.print( "GET /tally/" );
    stClient.print( bank );
    stClient.print( bankChan ); 
